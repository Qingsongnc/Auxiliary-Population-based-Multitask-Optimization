...
def GetPos(startpos, algorithm, fnc, ColPerT, LinePerT):
...
def outputTasks(sheet: Worksheet, Tasks, startpos=1, ColPerT=2, LinePerT=1, A=None):
...
def outputAlgorithm(sheet: Worksheet, Alg, AlgCom, startpos=1, ColPerT=2, LinePerT=1):
    assert not (ColPerT != 1 and LinePerT != 1), "The Multitask output is set on both lines and columns."
...
def outputData(sheet, A, Bs, startpos=1, ColPerT=1, LinePerT=2):
...
def outputSet(sheet, Tasks, A, Bs, filename1, filenames2, startpos=1, ColPerT=1, LinePerT=2):
    outputTasks(sheet, Tasks, startpos=startpos, ColPerT=ColPerT, LinePerT=LinePerT, A=A.shape[0])
    outputAlgorithm(sheet, filename1, filenames2, startpos=startpos, ColPerT=ColPerT, LinePerT=LinePerT)
    outputData(sheet, A, Bs, startpos, ColPerT, LinePerT)
...
def main(filename1, filenames2, algtype='22', T=2, LinePerT=1, outputFile=None, BestBold=False):
    # Create workbook
    ...
    if algtype == '22':
        ...
        outputSet(sheet, 'CEC2022', A, Bs, filename1, filenames2,
                  ColPerT=T, LinePerT=LinePerT)
    else:
...
if __name__ == '__main__':
    ...
    main(A, B, tasks, T=ColPerT, LinePerT=LinePerT, outputFile=outputFile)
